...
import {
  getMittwochmaedelsEventBySlug,
  getMittwochmaedelsEvents,
} from "@/lib/cms/getMittwochmaedelsEvents";
...
export async function generateMetadata({
  ...
  const event = await getMittwochmaedelsEventBySlug(slug);

  if (!event) return {};

  return {
    title: `${event.headline} - Mittwochsmädels - Sportfreunde Urlau e.V.`,
...
export default async function MittwochsmaedelsEventPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
